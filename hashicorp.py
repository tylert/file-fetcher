...
def main():
    ...
    consul-aws = find_latest(json.loads(r.text)['consul-aws']['versions'].keys())
    consul-esm = find_latest(json.loads(r.text)['consul-esm']['versions'].keys())
    consul-k8s = find_latest(json.loads(r.text)['consul-k8s']['versions'].keys())
    consul-replicate = find_latest(json.loads(r.text)['consul-replicate']['versions'].keys())
    consul-template = find_latest(json.loads(r.text)['consul-template']['versions'].keys())
    ...
    vault-ssh-helper = find_latest(json.loads(r.text)['vault-ssh-helper']['versions'].keys())

    fetch_hashicorp_files('consul', consul)
    fetch_hashicorp_files('consul-aws', consul-aws)
    fetch_hashicorp_files('consul-esm', consul-esm)
    fetch_hashicorp_files('consul-k8s', consul-k8s)
    fetch_hashicorp_files('consul-replicate', consul-replicate)
    fetch_hashicorp_files('consul-template', consul-template)
    ...
    fetch_hashicorp_files('vault-ssh-helper', vault-ssh-helper)
