...
func doIt() {
	...
	res, err := client.Get("https://api.github.com/repos/itchyny/gojq/releases/latest")
	if err != nil {
		log.Fatal(err)
	}
	...
	for i := 0; i < len(rel.Assets); i++ {
		if strings.Contains(rel.Assets[i].Name, "linux_arm64") {
			fmt.Println(rel.Assets[i].BrowserDownloadURL)
			fmt.Println("	dir=gojq")
			fmt.Println(fmt.Sprintf("	out=gojq-%s-linux-arm64.tar.gz", ver))
		} else if strings.Contains(rel.Assets[i].Name, "linux_amd64") {
			fmt.Println(rel.Assets[i].BrowserDownloadURL)
			fmt.Println("	dir=gojq")
			fmt.Println(fmt.Sprintf("	out=gojq-%s-linux-amd64.tar.gz", ver))
		} else {
	...
	fmt.Println("	dir=gojq")
	fmt.Println(fmt.Sprintf("	out=gojq-%s-src.tar.gz", ver))
}
