...
func main() {
	...
	res, err := client.Get("https://api.github.com/repos/scandum/tintin/releases/latest")
	if err != nil {
		log.Fatal(err)
	}
	...
	fmt.Println("# https://tintinplusplus-unoffical-documentation.readthedocs.io")

	// Compiled binaries
	for i := 0; i < len(rel.Assets); i++ {
		if strings.Contains(rel.Assets[i].Name, "tintin") {
			fmt.Println(rel.Assets[i].BrowserDownloadURL)
			fmt.Println("	dir=TinTin")
		} else {
	...
	fmt.Println("	dir=TinTin")
	fmt.Println(fmt.Sprintf("	out=tintin-src-%s.tar.gz", rel.TagName))
}
